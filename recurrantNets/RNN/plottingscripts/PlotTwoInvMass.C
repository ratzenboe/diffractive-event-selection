...
void PlotInvMass(TString fname, TString fname2, TString title1, TString title2)
{
    ...
    h_fd->Add((TH1F*)file->Get("invar_mass_full_recon"));
    TH1F* h_fd_2 = (TH1F*)file2->Get("invar_mass_feed_down");
    h_fd_2->Add((TH1F*)file2->Get("invar_mass_full_recon"));
    ...
    n_reduction.Form("%.2f", (integral_fd/ integral_fd_2));
    ...
    h_fd->SetLineColor(kBlack);
    h_fd_2->SetLineColor(kBlue);
    h_fd->GetXaxis()->SetTitle("M_{#pi#pi} [GeV]");
    h_fd->GetYaxis()->SetTitle("N_{evts}");
    h_fd->GetXaxis()->SetTitleOffset(1.1);
    h_fd->GetYaxis()->SetTitleOffset(1.15);
    h_fd->GetYaxis()->SetTitleSize(0.06);
    h_fd->GetXaxis()->SetTitleSize(0.06);
    h_fd->GetXaxis()->SetLabelSize(0.045);
    h_fd->GetYaxis()->SetLabelSize(0.045);
    /* h_sum->SetTitle(title); */

    h_fd->Draw("P");
    h_fd_2->Draw("sameP");

    h_fd->SetMarkerStyle(kFullTriangleUp);
    h_fd_2->SetMarkerStyle(kFullTriangleDown);

    h_fd->SetMarkerSize(1.2);
    h_fd_2->SetMarkerSize(1.2);

    h_fd->SetMarkerColor(kBlack);
    h_fd_2->SetMarkerColor(kBlue);

    TLegend* leg = new TLegend(0.591336,0.650505,0.922756,0.849495);
    leg->SetHeader(("Reduction: "+n_reduction).Data());
    leg->AddEntry(h_fd, (title1).Data(),"p");
    leg->AddEntry(h_fd_2, (title2).Data(),"p");
    /* leg->AddEntry(h_fd, "LHC16 !AD","p"); */
    /* leg->AddEntry(h_fd_2, "MVA cut","p"); */

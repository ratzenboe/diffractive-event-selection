...
from keras.layers import Masking, LSTM, GRU, Input, BatchNormalization, Flatten, AveragePooling1D, MaxPooling1D, Conv1D, Bidirectional, AlphaDropout
...
def train_model(data, run_mode_user, val_data,
    ...
    if ('emcal' not in key_lst and 'selu' not in activation):
        ...
        history = selu_net(data, val_data, batch_size, n_epochs, rnn_layer,
                           out_path, dropout, n_layers, layer_nodes, batch_norm, 
                           k_reg, activation, aux, flat)
        return history
...
def selu_net(data, val_data, batch_size=64, n_epochs=30, rnn_layer='LSTM', 

...
\begin{frame}
    ...
    \end{itemize}
    \centering\begin{tikzpicture}
        \node<2->[inner sep=0pt] (cep) at (0,0) {\includegraphics[height=3cm,keepaspectratio]{pics/CEP1.pdf}};
        % \node[align = center, above = of cep, yshift=-15mm] {\small CEP};
...
\begin{frame}
    \frametitle{Decision tree learning}
    1) We compute a measure for \emph{each possible split} in each feature\\\hspace*{5mm}$\to$ here \textbf{absolute error rate} (AER)\\
    2) Recursively repeat step (1) for each subspace until AER $\to 0$
    \\
    \vspace*{3mm}
...
\begin{frame}
    ...
    \begin{itemize}
        \item Use more sophisticated split measures 
            \begin{itemize}
                \item \emph{Information gain} $\leftrightarrow$ (im-)purity of split sub-sets
            \end{itemize}
        \item Pruning 
...
% % -----------------------------------------------
\begin{frame}
    \frametitle{Update weights via back-propagation}
...
\begin{frame}
    ...
    \begin{itemize}
        \item<1-> Currently: emcal data are not making a difference\\\hspace*{5mm}$\to$~however, they should!

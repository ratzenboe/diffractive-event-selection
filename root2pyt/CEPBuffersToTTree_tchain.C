        ...
        Int_t evt_offset, Int_t n_evts_to_read, TString file_addon_str="")
{
    ...
    Double_t CC_E, CC_shapeDispersion, CC_chi2, CC_CPVDist, CC_pos_x, CC_pos_y, CC_pos_z, 
             CC_pos_eta, CC_pos_phi, CC_pos_pt, CC_m02, CC_m20, CC_time, CC_dphieta;
    Bool_t CC_hastrkmatch, CC_isBGcluster;
    ...
    caloClusterTree->Branch("dPhiEta", &CC_dphieta);
    caloClusterTree->Branch("hasTrkMatch", &CC_hastrkmatch);
    caloClusterTree->Branch("isBGcluster", &CC_isBGcluster);
    ...
    Int_t cluster_filled(0), cluster_total(0), cluster_avail(0);
    //
    ...
    for (UInt_t ii(evt_offset); ii<evt_offset+n_evts_to_read; ii++)
    {
        ...
        for (UInt_t kk(0); kk<cep_raw_evt->GetnCaloClusterTotal(); kk++)
        {
            ...
            cluster_avail++;
            // put here all track info
            /* printf("GetCaloClusterIsEMCAL(): %s\n", (rawCaloCluster->GetCaloClusterIsEMCAL()) ? "true" : "false"); */
            /* printf("GetCaloClusterIsPHOS(): %s\n", (rawCaloCluster->GetCaloClusterIsPHOS()) ? "true" : "false"); */
            if (!rawCaloCluster->GetCaloClusterIsEMCAL()) continue;
            /* if (IsClusterFromBG(0.51)|| ) continue; */
            CC_dphieta = rawCaloCluster->GetCaloClusterEtaPhiDistNearTrk();
            CC_hastrkmatch = rawCaloCluster->GetCaloClusterHasTrkToMatch();
            CC_isBGcluster = rawCaloCluster->IsClusterFromBG(0.51);
            if (CC_dphieta>100.) continue;
    ...
    /* h->Draw(); */
    /* c->SaveAs("dphieta.pdf"); */
    // cursor of status display has to move to the next line
    std::cout << std::endl;
    std::cout << "100%:   conversion finished " << std::endl;
    printf("Cluster information has been filled %i times\n", cluster_filled);
    printf("Number of clusters available: %i\n", cluster_avail);
    printf("Number of total clusters: %i\n", cluster_total);
    ///////////////////////

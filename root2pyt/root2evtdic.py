...
def root2pandas(filename, treename):
    """
    uses uproot to import the TTree from the filename transforming it into 
    a pandas dataframe
    """
    df = uproot.open(filename)[treename].pandas.df()
    # the column names are saved in a byte-string making the keys in the
    # dataframe not accessable; this is fixed with the next line
...
def main():
    ...
    try:
        ...
        print_dict(max_entries_dic, '::  max_entries_dic')
        print_dict(cut_dic, '::  cut_dic')
    except KeyError:
    ...
    for i in range(base, num_paths):
        ...
        data = root2pandas(filespath+temp_path_dic[event_string], event_string)
        ...
        for key in temp_path_dic.keys():
            ...
            print('key of temp_path_dic: {}'.format(key))
            data = root2pandas(filespath + temp_path_dic[key], key)
            ##############################################################################
